package openjdk.tools.json.util.generic;
...
public class JsonStringUtility {
	 ...
	 * JsonMap my_tag_values = new JsonMap();
	 ...
	 * @param tag_values   : the JSON object containing the hash tag values to be
	 *                     applied (key/value = tag-name/replacement-value)
	 * @return Returns the updated String
	 */
	public static String updateHashTags(String input_string, JsonMap tag_values) {
		String output = input_string;
		for (String tag : tag_values.keySet()) {
			output = output.replace("#" + tag + "#", String.valueOf(tag_values.get(tag)));
		}
		return output;
	}
	 ...
	 * @return Returns the updated String
	 */
	public static String updtateHashTags(InputStream input_stream, JsonMap tag_values) {
	 ...
	 * object using the object's keys as the tag-names. Tags must be in the form of
	 * TAG_PREFIX.TAG_NAME.TAG_SUFFIX (e.g. if tag_prefix="{@literal <TAG>}" and
	 * tag_suffix="{@literal </TAG>}" the the tag is
	 * {@literal <TAG>}my_tag{@literal </TAG>})
	 * 
	 * <b>Example:</b> String my_tagged_string = "I would just like to say.. #tag_1#
	 * #tag_2# !!!"; JsonMap my_tag_values = new JsonMap();
	 * my_tag_values.put("tag_1", "Hello"); my_tag_values.put("tag_2", "world");
	 * System.out.println(JSONStringUtility.updateStringTags(my_tagged_string,
	 * "{@literal <TAG>}", "{@literal </TAG>}", my_tag_values)); <b>Output:</b> I
	 * would just like to say.. Hello world !!!
	 * 
	 * <pre>
	 * 
	 ...
	 * @param tag_values   : the JSON object containing the tag values to be applied
	 *                     (key/value = tag-name/replacement-value)
	 * @return Returns the updated String
	 */
	public static String updateStringTags(String input_string, String tag_prefix, String tag_suffix,
			JsonMap tag_values) {
		String output = input_string;
		for (String tag_name : tag_values.keySet()) {
			output = output.replace(tag_prefix + tag_name + tag_suffix, String.valueOf(tag_values.get(tag_name)));
		}
		return output;
	}
	 ...
	 * @return Returns the updated String
	 */
	public static String updateStringTags(InputStream input_stream, String tag_prefix, String tag_suffix,
			JsonMap tag_values) {
