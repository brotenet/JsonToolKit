...
public class JsonPojoWriter implements Closeable, Flushable {
	...
	private final Map<Class, JsonClassWriterBase> writers = new HashMap<Class, JsonClassWriterBase>(BASE_WRITERS); // Add
																													// customer
																													// writers
																													// (these
																													// make
	...
	public static String objectToJson(Object item, Map<String, Object> optionalArgs) {
		try {
			...
			throw new JsonInputOutputException("Unable to convert object to JSON", e);
		}
	}
	...
	public JsonPojoWriter(OutputStream out, Map<String, Object> optionalArgs) {
		...
		if (optionalArgs.containsKey(FIELD_SPECIFIERS)) {
			...
			for (Entry<Class, List<String>> entry : specifiers.entrySet()) {
				...
				for (String field : fields) {
					...
					if (f == null) {
						throw new JsonInputOutputException("Unable to locate field: " + field + " on class: "
								+ c.getName()
			...
			for (Entry<Class, List<String>> entry : blackList.entrySet()) {
				...
				for (String field : fields) {
					...
					if (f == null) {
						throw new JsonInputOutputException("Unable to locate field: " + field + " on class: "
								+ c.getName()
								+ ". Make sure the fields in the FIELD_NAME_BLACK_LIST map existing on the associated class.");
					}
					newList.add(f);
				}
				copy.put(c, newList);
		...
		try {
			...
			throw new JsonInputOutputException("UTF-8 not supported on your JVM.  Unable to convert object to JSON.",
					e);
		}
	}
	...
	public boolean writeIfMatching(Object o, boolean showType, Writer output) {
		...
		try {
			...
			throw new JsonInputOutputException("Unable to write custom formatted object:", e);
		}
	}
	...
	public boolean writeArrayElementIfMatching(Class arrayComponentClass, Object o, boolean showType, Writer output) {
		...
		try {
			...
			throw new JsonInputOutputException("Unable to write custom formatted object as array element:", e);
		}
	}
	...
	public void write(Object obj) {
		...
		try {
			...
			throw new JsonInputOutputException("Error writing object to JSON:", e);
		}
		flush();
		objVisited.clear();
	...
	protected void traceFields(final Deque<Object> stack, final Object obj,
			final Map<Class, List<Field>> fieldSpecifiers) {
		...
		for (final Field field : fields) {
			...
			try {
				...
				if (o != null && !JsonPojoMetaUtils.isLogicalPrimitive(o.getClass())) { // Trace through objects that
	...
	private static List<Field> getFieldsUsingSpecifier(final Class classBeingWritten,
			final Map<Class, List<Field>> fieldSpecifiers) {
	...
	private void writeJsonObjectObject(JsonPojoElement jObj, boolean showType) throws IOException {
		...
		if (showType) {
			...
			output.write('"');
			try {
				type = JsonPojoMetaUtils.classForName(jObj.type, getClassLoader());
			} catch (Exception ignored) {
				type = null;
			}
		}
	...
	public void writeObject(final Object obj, boolean showType, boolean bodyOnly) throws IOException {
		...
		final List<Field> fieldBlackListForClass = getFieldsUsingSpecifier(obj.getClass(),
				(Map) args.get(FIELD_BLACK_LIST));
